@echo off
rem bin/cg-autopilot-control.cmd -- Compound GPID autopilot control helper (Windows)
rem
...
rem
rem The helper resolves its code from this installation directory and takes
rem the validated consumer project root through an explicit --root argument.
rem It never changes the working directory to the package checkout and never
rem selects consumer tests from the installed source test inventory.
rem
...
:run_python
call %PYTHON_CMD% "%~dp0..\scripts\cg_autopilot.py" %*
exit /b %ERRORLEVEL%
