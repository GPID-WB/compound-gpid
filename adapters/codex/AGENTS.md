...
behavior of GitHub Copilot. It only tells Codex-compatible agents how to read
and execute the existing GitHub Copilot-oriented assets.
...
unavailable tool names to Codex behavior:
...
- Task/Subagent/Parallel: run sequentially in the main thread unless a native
  subagent tool is available; use native parallel tool calls only for
  independent tool calls
- TaskCreate/TaskUpdate/TaskList/TaskGet/TaskStop/TaskOutput and
  TodoWrite/TodoRead: use the available plan/task tracking tool
