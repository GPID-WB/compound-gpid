...
- **Python regex raw strings: never use `\\r`/`\\n` in character classes — use `\r`/`\n` or hex escapes**: In Python raw strings, `r'[^\\r\\n]'` does NOT exclude carriage-return and newline. The regex engine sees `\\r` as escaped-backslash + `r`, making it exclude the **letters** `r` and `n` instead. This is a silent data-quality bug: any string value containing `r` or `n` will fail to match. Rule: use `r'[^\r\n]'` (single backslash) for line terminators, and `\x27` hex-escape for apostrophe instead of `\'` to avoid the backslash-letter collision. Applies especially to frontmatter parsers embedded in bash heredocs where escaping layers compound the confusion. See `.cg-docs/solutions/bugs/2026-05-14-python-regex-raw-string-double-backslash-excludes-letters.md`.
