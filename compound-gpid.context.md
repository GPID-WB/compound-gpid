...
- **Cross-agent native platform trees are generated from `.github/` canonical source**: `.claude/`, `.agents/`, and `.opencode/` are committed generated trees produced by `scripts/cg_generate_targets.py` from `.github/shared/target-mapping.json`. `/cg-commit-push-pr` auto-regenerates them when `.github/` files change; `cg-update` regenerates after pull. Drift tests fail if generated trees are stale. The prior `adapters/` package is superseded. See `.cg-docs/solutions/environment-issues/2026-07-03-cross-agent-native-platform-trees-require-generator-drift-tests-consistent-python.md`.

- **Platform linking uses per-unit install definitions, not whole-root native links**: `cg-link` defaults to all supported platforms (`copilot`, `claude-code`, `codex`, `opencode`) and applies directory/file units under `.github/`, `.claude/`, `.agents/`, and `.opencode/`. Directory units are junctions on Windows and symlinks on macOS. Real user-owned directories/files are skipped with warnings so one conflict does not block other selected platform units. `cg-unlink` discovers managed units from target mapping, symlink/junction ownership, and `.compound-gpid/managed-files.json`; it must work even when `.github/` is absent (for example OpenCode-only installs).

- **Strict platform config files use sidecar manifest checksums, not inline markers**: JSON config files such as `.opencode/opencode.json` cannot contain marker comments. Managed copied files are tracked in `.compound-gpid/managed-files.json` with source path and SHA-256 checksum. `cg-update` refreshes a copied file only if its current checksum still matches the manifest; user-modified files are skipped and left in place. `cg-unlink` removes copied files only when the checksum still matches and otherwise drops CG ownership.
...
- **Hyphen in regex character class creates invalid range unless at start or end**: `[A-Za-z0-9_. :/-]` has the sequence `/-` which is an invalid range in POSIX, Python `re`, and JavaScript (ASCII 47 > 45). PowerShell's .NET engine is permissive and treats it as two literals — the regex "works" in tests but silently breaks in any strict-engine environment. Always place `-` at the very start `[-...]` or very end `...-]` of a character class, never between two other characters. LLMs authoring regex in agent prose are prone to this: listing special chars in "natural order" places `-` mid-class. Canonical safe form for "letters, digits, and `_.:/ -`": `^[-A-Za-z0-9_. :/]*$`. See `.cg-docs/solutions/bugs/2026-06-11-invalid-regex-character-range-hyphen-placement.md`.
...
- **Model-guide drift checks need semantic equivalence for inherited prompts**: Ordinary prompts that inherit the GitHub Copilot model picker intentionally omit `model:` frontmatter, while docs may describe them as `Copilot model picker`. Audit drift checks must treat that exact inherited/catalog role as equivalent to missing frontmatter, and must still flag real target-state drift for explicit model assignments. See `.cg-docs/solutions/testing-patterns/2026-06-15-inherited-model-picker-drift-equivalence.md`.

- **Token-audit warning closure requires reviewed classifications**: For token/context optimization work, do not treat `warnings = 0` as the only success state and do not treat all warnings as harmless. The durable closure gate is `failures = 0` plus reviewed warning counts with `fix = 0`; remaining warnings must be classified as accepted maintenance/safety reads or docs-only references, tested in `scripts/tests/test_audit_context.py`, and reflected in regenerated `.cg-docs/cost/` docs. See `.cg-docs/solutions/testing-patterns/2026-06-16-reviewed-warning-classifications-close-token-work.md`.
...
- **Workflow telemetry needs source-aware extraction tests**: When `scripts/cg_audit_context.py` adds workflow-level metrics, do not reuse narrow legacy regexes unless their measurement contract matches the new field. Add fixture tests with realistic prompt idioms (backticked paths, Windows slashes, shared contracts, and tool names like `execution_subagent`) and verify the generated JSON row for the affected workflow. See `.cg-docs/solutions/testing-patterns/2026-06-22-workflow-telemetry-source-path-tool-extraction.md`.
...
- **Bash Python wrappers must probe `python3`, `python`, then `py`**: macOS usually has `python3`, but repo wrappers and scripts must use the same detection order as Windows (`python3 -> python -> py`) and verify `--version` starts with `Python`. Do not hard-code `python3` for generator, profile-edit, managed-manifest, or summary/index/audit wrapper calls. This keeps behavior consistent with PowerShell `Resolve-PythonCommand` and Windows `.cmd` launchers.

## PowerShell Conventions

- **Never call `python3` directly in PowerShell scripts — use `Resolve-PythonCommand`**: Windows may have `python` or `py` but not `python3`. The `Resolve-PythonCommand` helper in `scripts/helpers.ps1` probes `python3 -> python -> py` with Windows Store stub rejection, matching `install.ps1`'s `Test-PythonCandidate` and `bin/*.cmd`'s `where/for-f/findstr` pattern. Always use it instead of bare `python3` in any `.ps1` script that invokes Python. See `.cg-docs/solutions/environment-issues/2026-07-03-cross-agent-native-platform-trees-require-generator-drift-tests-consistent-python.md`.
...
- **Prompts that invoke CLI tools must specify shell-safe argument construction explicitly**: LLM agents do not automatically quote flag values — they substitute raw strings. `--title "<value>"` with an embedded `"` breaks shell argument splitting. `--label <labels>` with a space in a label injects extra CLI flags. Prevention checklist for any prompt invoking `gh`, `git`, or similar: (1) quote each `--flag "..."` value individually; (2) strip shell metacharacters (`"`, `` ` ``) from titles before `--title`; (3) pass multi-label as separate `--label "..."` flags, not a single concatenated string; (4) add a realpath canonical-path check after string-based path validation to block symlink traversal; (5) gate mode-conditional hard-stops before the stop — not in a carve-out after it (agents read sequentially). Use structural fencing (```` ```text ```` blocks, `--body-file`) rather than keyword blocklists for untrusted content. See `.cg-docs/solutions/bugs/2026-06-11-cli-injection-in-llm-driven-gh-prompts.md`.

- **Realpath/canonical-path checks in prompts must require a tool call — prose reasoning is insufficient**: An instruction like "resolve the canonical path (following symlinks)" reads as *reasoning* to an LLM, not *execution*. The agent reasons from the string representation of the path and concludes it is safe — without ever following the symlink. An OS-level syscall is required. Use explicit language: "Execute `Resolve-Path` (PowerShell) or `readlink -f` (bash/Linux) **via a tool call**; compare the returned string. String-only reasoning is insufficient." See `.cg-docs/solutions/bugs/2026-06-11-llm-prose-only-syscall-is-unenforceable.md`.

- **Structural fencing (fenced `\`\`\`text\`\`\`` blocks) can be broken out of by untrusted content containing ` ``` `**: Markdown fences close on the first ` ``` ` encountered, regardless of whether it is "inside" the block semantically. Plan files and roadmap descriptions routinely contain code fences. Before embedding untrusted content in a fenced block, replace every ` ``` ` occurrence with `` ` ` ` `` (three backticks separated by spaces). Document this escaping step in both the operational step and the Safety Rules summary. See `.cg-docs/solutions/testing-patterns/2026-06-11-fenced-block-delimiter-collision-in-untrusted-content.md`.

- **"Warn before proceeding" is not a TOCTOU guard — use explicit stop + enumerated choices**: An LLM executing a prompt step that says "warn the user before proceeding" emits the warning text and then proceeds to the next instruction. This is not a stop. For any risky decision point (duplicate detected, destructive action, irreversible state change), use: "Stop immediately. Present the user with three choices: (a) ..., (b) ..., (c) .... Do NOT dispatch/write/create until the user responds." Enumerate choices; name the forbidden action explicitly. Anti-patterns: "warn", "ask", "confirm" without a corresponding stop instruction. See `.cg-docs/solutions/bugs/2026-06-11-ambiguous-warn-before-proceeding-toctou-guard.md`.
...
- **Browser evidence manifests must require every referenced file**: Capture must fail before writing a manifest when a referenced source, view, PDF, or screenshot is missing or empty. Validate existence, non-empty content, and SHA-256 unconditionally; never hide a missing artifact behind an existence guard. See `.cg-docs/solutions/testing-patterns/2026-08-10-evidence-manifest-tests-require-referenced-files.md`.

- **GitHub workflow dispatch must fail closed and verify mutation responses**: Select a live branch only for an exact explicit opt-in; every other value stays dry-run. A successful CLI exit does not prove a GitHub mutation persisted; parse the response and validate its expected shape, treating present-but-wrong data as an API/configuration error. See `.cg-docs/solutions/testing-patterns/2026-08-11-workflow-dispatch-boolean-fail-open-verify-mutations.md`.

- **Removed managed install units require checksum-guarded migration**: When a generated or copied asset is removed from `target-mapping.json`, both link scripts must inspect prior managed-file manifests, remove only unchanged legacy files, preserve user-modified files, and drop stale ownership records. Add parity assertions and upgrade fixtures for both outcomes. See `.cg-docs/solutions/testing-patterns/2026-07-31-advisory-inheritance-audit-and-legacy-cleanup.md`.

- **Kilo requires project-local markdown sources**: Kilo install units must use checksum-managed `copy-directory` semantics rather than junctioning markdown outside the consumer project. Keep the strategy identical in PowerShell and POSIX link flows, with mapping-level parity tests. See `.cg-docs/solutions/bugs/2026-08-11-windows-link-kilo-copy-directory-parse-failure.md`.

- **Inheritance audits distinguish absent metadata from explicit nulls**: Check key presence (`"model" in frontmatter`) so `model: null` cannot evade execution-metadata validation. Validate malformed advisory data defensively and keep runtime model availability explicitly unverified. See `.cg-docs/solutions/testing-patterns/2026-07-31-advisory-inheritance-audit-and-legacy-cleanup.md`.

- **CLI integrations validate response shape, not only JSON syntax**: A valid JSON response with wrong field types can raise uncaught extraction errors and violate a documented exit-code contract. Guard each extraction block, distinguish deliberate absence from malformed present data, and add wrong-shape regression fixtures for every remote call site. See `.cg-docs/solutions/bugs/2026-08-10-typed-invalid-gh-cli-payloads-crash-exit-code-contract.md`.
