    ...
    if (Test-Path $kiloConfigPath) {
        ...
        if (-not [string]::IsNullOrWhiteSpace($raw)) {
            try {
                ...
                Write-Warning "  Could not parse kilo.jsonc; will overwrite with valid JSON."
                $config = @{}
            }
        }
    }

    if (-not $config.ContainsKey("permission")) {
        $config["permission"] = @{}
    }
    if (-not ($config["permission"] -is [hashtable])) {
        $config["permission"] = @{}
    }
    if (-not $config["permission"].ContainsKey("markdown_source")) {
        $config["permission"]["markdown_source"] = @{}
    }
    if (-not ($config["permission"]["markdown_source"] -is [hashtable])) {
        $config["permission"]["markdown_source"] = @{}
    }

    if ($Remove) {
        if ($config["permission"]["markdown_source"].ContainsKey($permissionKey)) {
