...
- `.github/shared/model-catalog.json` is the source of truth for known model
  names, vendors, families, roles, frontmatter support status, and prompt/agent
  assignments.
- `docs/model-guide.md` must match the catalog and explain any intentional
  exception to OpenAI-first routing.
- Inherited model-picker prompts should omit `model:` in prompt frontmatter.
  The guide may describe them as `Copilot model picker`; the audit treats that
  as equivalent only when the catalog role is `inherited`.
- GPT model strings visible in the Copilot model picker are not enough for
  release readiness. Validate each exact string in VS Code/Copilot frontmatter
  before rewriting broad prompt or agent defaults.
- When code was generated by an Anthropic model, prefer an OpenAI review model
