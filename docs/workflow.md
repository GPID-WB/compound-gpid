...
- *Team member onboarding*: The committed `compound-gpid.md` charter and `compound-gpid.local.md` config are already in the repo. A new member runs `/cg-setup` only if they need to adjust local preferences.
...
Use this before merging or releasing prompt, model-advisory, context-loading,
...
- `.github/shared/model-advisory.contract.md` is the canonical source for stage
  capability profiles, effort labels, source fallback, provenance, and user
  control.
- `.github/shared/model-advisory-examples.json` contains dated examples that are
  explicitly availability-unverified and secondary to capability-only advice.
- Canonical prompts and agents omit executable model metadata; generated targets
  preserve that inheritance and do not emit standalone mapping artifacts.
- `docs/model-guide.md` must describe process-stage capability needs, strong and
  economical options, availability caveats, Auto/unknown behavior, and the fact
  that the user makes the final selection.
- Cross-family review may be suggested when the generator family is known. When
  the family is unknown or Auto, do not infer the hidden identity or claim that
  a cross-family review occurred.
- Runtime availability, picker behavior, and live catalog discovery remain
  unverified or deferred unless observed through a supported platform mechanism.
...
- Run `/cg-token-audit` from a linked project when you want a compact advisory
  summary of token/context usage. It runs deterministic tooling with
  `--root .`, writes reports under `.cg-docs/cost/`, and summarizes
  `token-advice.md`.
- Treat `fix` warnings as candidates for prompt/context slimming, `accept`
  warnings as intentional maintenance or safety reads, and `docs-only`
  warnings as reference wording that does not imply runtime broad loading.
- Use the advice to match capability and effort to the process stage, prioritize
  successful completion before token economy, and avoid broad context reads
  unless a workflow explicitly justifies them.
...
- Canonical and generated prompts/agents contain no executable model metadata.
- Advisory examples remain dated and availability-unverified; runtime behavior is
  labeled separately from static evidence.
...
- Manual VS Code/Copilot validation covers picker inheritance, `/cg-plan`,
  `/cg-work review:auto`, `/cg-work review:manual`, `/cg-review light`,
  `/cg-review data-risk`, `/cg-review full`, and Knowledge Brain selective
  retrieval.
