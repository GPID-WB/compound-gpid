...
| `cg-skill-stata-testing` | Stata testing & reproducibility patterns: inline assertions (`assert`, `capture`, `_rc`, exit codes), data validation (`isid`, `duplicates`, `misstable`), econometric result verification (`_b[]`, `reldif`, `test`), `reprun`/`repkit` reproducibility workflows, test scaffolding (`foreach`, `preserve`/`restore`), and 8 testing-specific anti-patterns. Load when writing, reviewing, or debugging test blocks in `.do`/`.ado` files. |
