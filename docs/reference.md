...
| `/cg-compound` | Claude Sonnet 4.6 | Capture solutions as reusable knowledge in `.cg-docs/solutions/`. Cross-references related existing solutions. Automatically updates the project wiki (`wiki/`) when the captured solution has user-facing implications. Use `--propose` to review proposed wiki changes before they are applied. |
...
| `/cg-wiki [rebuild\|restructure\|convert\|status\|help] [--propose]` | Claude Sonnet 4.6 | Manage the project wiki (`wiki/` by default). No args = status table. `rebuild` regenerates all auto-managed pages from current codebase + charter. `rebuild <page-id>` targets a single page. `restructure` lets you add/remove/reorder pages interactively. `convert` generates GitHub Wiki–compatible layout (Home.md, _Sidebar.md). `--propose` shows diffs before writing. Wiki initialized at `/cg-setup`; updated automatically by `/cg-compound`. |
