...
and distributed via the same junction/symlink mechanism as `.github/`.
...
Link platform trees with `cg-link --platforms copilot,claude-code,codex,opencode`.
Default (`cg-link` with no `--platforms` flag) links only `.github/` for Copilot,
