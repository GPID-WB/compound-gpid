...
Describe "update.ps1 - copilot-instructions.md refresh" {
    Context "when the file in CWD has the management marker" {
        It "regenerates the file with updated content (template-based, not copied from source)" {
            $marker  = "<!-- compound-gpid:managed -->"
            $dest    = Join-Path $TestDrive "copilot-managed.md"

            # Simulate a managed file (marker present = was generated by cg-link)
            Set-Content -Path $dest -Value ($marker + "`n# Old generated content")

            $content = Get-Content $dest -Raw
            $content -match [regex]::Escape($marker) | Should Be $true

            # Simulate regeneration: marker present -> overwrite with new generated content
            Set-Content -Path $dest -Value ($marker + "`n# Regenerated content")
            (Get-Content $dest -Raw) -match "Regenerated content" | Should Be $true
        }
    }

    Context "when the file in CWD does NOT have the management marker" {
        It "leaves the user-managed file untouched (no marker = user-owned, not generated)" {
            $marker  = "<!-- compound-gpid:managed -->"
            $dest    = Join-Path $TestDrive "copilot-user.md"

            Set-Content -Path $dest -Value "# My custom instructions (no marker)"

            # Simulate the marker check - should NOT regenerate
            ...
            $result -match "Regenerated content"     | Should Be $false
        }
    }
