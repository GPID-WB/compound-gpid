...
- `.cg-docs/solutions/bugs/2026-05-20-fix-helper-written-but-not-wired-into-call-site.md` — same class of bug: protective logic written but positioned at the wrong layer (helper defined but call site never updated)
- `.cg-docs/solutions/testing-patterns/2026-05-29-warning-emission-tests-need-assertwarns-verify-pass-catches-gap.md` — companion pattern: when adding `warnings.warn()`, always add `assertWarns`/`pytest.warns` to the test; verify pass reliably catches this gap
