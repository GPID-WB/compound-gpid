...
- [`.cg-docs/solutions/bugs/2026-05-07-python-nonatomic-path-write-use-mkstemp-replace.md`](./2026-05-07-python-nonatomic-path-write-use-mkstemp-replace.md) — Python-specific variant: `Path.write_text()` truncates the destination before writing; use `mkstemp + os.replace()` for crash-safe atomic output
- [`.cg-docs/solutions/bugs/2026-08-14-capability-eligibility-namespace-mismatch.md`](./2026-08-14-capability-eligibility-namespace-mismatch.md) — adjacent validation discipline: a manifest/selection layer must fail closed on malformed or mismatched input (namespace mismatch, unknown ids, platform-id typos) rather than silently preserving the wrong state
- [`.cg-docs/solutions/data-quality/2026-08-28-exact-json-registry-mutation-boundaries.md`](../data-quality/2026-08-28-exact-json-registry-mutation-boundaries.md) — extends validate-before-persist to exact unknown JSON values, rendered-size limits, and pre-commit response validation
