...
This contract governs the goal-driven execution loop used by `/cg-plan`,
`/cg-work`, and qualifying `/cg-light-work` tasks. `/cg-plan` and
`/cg-light-work` produce completion contracts in saved plans. `/cg-work` and the
bounded `/cg-light-work` execution stage record evidence in durable execution
reports. Completion is recorded only when required evidence is present or
explicitly accepted as an exception.
...
4. `.opencode/shared/review-routing.contract.md` review routing.
...
preflight defined in `.opencode/shared/artifact-view.contract.md` before roadmap,
...
3. Lightweight plans generated by `/cg-light-work` use the current Plan schema,
   include the completion contract, and default to `deviation-policy: ask`.
