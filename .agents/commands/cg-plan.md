...
3. Load `.agents/shared/context-loading.contract.md` and apply Stage 0/1/2 first. Do not read full `compound-gpid.context.md` by default; if the plan topic needs tactical project facts, search headings or snippets and state `Context expansion: reading <artifact/section> because <reason>.`
...
`.agents/shared/artifact-view.contract.md` and validate the saved source:
...
For Deep phased plans, add an optional `Phase` column to Verification Surface and Constraints tables. For Lightweight plans, a condensed contract (Outcome + Verification Surface only) is acceptable. See `.agents/shared/goal-execution.contract.md` for full schema details.
