...
- PowerShell/CMD style: `Set-StrictMode -Version Latest`, `$PSScriptRoot` for paths. For `bin/*.cmd` Python launchers, always load `cg-skill-windows-cmd-python-detection` — the skill covers the mandatory `where` pre-check pattern, the parity rule, and required test assertions. The `powershell.instructions.md` file (auto-applied to `.ps1` and `.cmd` files) captures these as coding standards.
