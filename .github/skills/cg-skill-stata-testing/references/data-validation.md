...
assert dup_flag == 0, "Dataset is not uniquely identified by country_code + year"
```
...
assert inrange(welfare_ppp, 0, .)     // non-negative welfare
...
xtset country_code year
assert e(balanced) == "strongly balanced"    // after xtset, inspect macro

* Alternative: assert using xtdescribe
quietly xtdescribe
assert r(min) == r(max)    // all panels have the same span
```
