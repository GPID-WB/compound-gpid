...
- Stata: `compress`, `quietly` in loops, `mata` for compute-heavy operations, `gcollapse` (gtools) for large-N grouped operations, efficient `bysort`/`egen` patterns. Load `cg-skill-stata-core` before reviewing any `.do` or `.ado` file; additionally load `cg-skill-stata-research` when reviewing analytical do-files.
