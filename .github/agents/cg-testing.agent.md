...
- Stata: `assert` statements, validation do-files, `datasignature`, `reprun`. Load `cg-skill-stata-core` before reviewing any `.do` or `.ado` file; additionally load `cg-skill-stata-research` when reviewing analytical do-files (poverty, welfare, econometrics).
