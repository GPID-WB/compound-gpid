...
- Stata: `*!` version comments in `.ado` files, standard do-file header blocks, units comments on `replace`. Load `cg-skill-stata-core` before reviewing any `.do` or `.ado` file.
